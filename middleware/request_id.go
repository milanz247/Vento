...
// honored; otherwise a fresh random one is generated. The value is written
// onto both the request headers - so downstream handlers can read it via
// c.Request.Header.Get("X-Request-ID") - and the response headers.
//
...
// newID returns a 16-byte random identifier as a 32-char hex string. It
// falls back to a fixed sentinel only if the system CSPRNG is unavailable,
